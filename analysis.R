...
protests <- read.csv("https://countlove.org/data/data.csv", stringsAsFactors = F)
...
count_in_location <- function(location){
  N <- sum(str_count(protests$Location, paste0(".*", location,".*")))
...
# `stringr` package
states <- str_sub(protests$Location, start = -2, end = -1)
...
max_in_state <- max(state_table, na.rm =TRUE)
...
num_may_24 <- count_on_date(as.Date("24may2020", "%d%b%Y"))

# Using your function you just wrote, how many protests were there on 
# May 31th, 2020? `num_on_may_31`
num_may_31 <- count_on_date(as.Date("31may2020", "%d%b%Y"))
...
change_july_protests <- table(months(as.Date(in_2020)))[["July"]][1] - table(months(as.Date(in_2019)))[["July"]][1]
...
matched_high <- str_match(purpose, "(.*) \\(.*\\)")[,2]
high_level_purpose <- matched_high[complete.cases(matched_high)]
...
protests_change_between <- function(year1, month1, year2, month2) {
  in_year1 <- protests$Date[str_detect(dates, year1)]
  in_year2 <- protests$Date[str_detect(dates, year2)]
  abs(table(months(as.Date(in_year2)))[[month2]][1] - table(months(as.Date(in_year1)))[[month1]][1])
}

protests_change_between("2020", "July", "2019", "July")
